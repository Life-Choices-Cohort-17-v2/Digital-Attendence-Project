...
$user = require_auth('staff'); // ensures we have id, name, email, employeeId, role
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        /* (all your existing dashboard styles – keep as is) */
        ...
        .calendar-day.selected { background: var(--olive-green); color: var(--sidebar-blue); border-color: var(--olive-green); }
        .calendar-day.other-month { opacity: 0.3; }
        .calendar-nav { display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; }
        .calendar-nav button { padding: 8px 16px; background: var(--card-bg); border: 1px solid var(--border-color); border-radius: 12px; cursor: pointer; }
        .selected-date-info { margin-top: 20px; padding: 16px; background: var(--olive-green-soft); border-radius: 12px; text-align: center; }
        .input-group { margin-bottom: 10px; }
        .input-group label { display: block; font-size: 13px; font-weight: 500; margin-bottom: 6px; color: var(--heading); }
        .input-group input, .input-group select, .input-group textarea { width: 100%; padding: 10px 14px; border: 1px solid var(--border-color); border-radius: 12px; background: var(--card-bg); color: var(--heading); font-family: inherit; font-size: 13px; }
...
<div x-data="dashboardApp()" x-init="init()" @keydown.escape="sidebarOpen = false" x-cloak>
    ...
    <div x-show="calendarModalOpen" x-cloak class="modal-overlay" @click.away="calendarModalOpen = false">
        <div class="modal-container">
            <div class="modal-header"><h2>Calendar</h2><button class="modal-close" @click="calendarModalOpen = false">✕</button></div>
            ...
            <div class="modal-footer"><button class="btn-cancel" @click="calendarModalOpen = false">Close</button></div>
        </div>
    </div>
    ...
    <div x-show="leaveModalOpen" x-cloak class="modal-overlay" @click.away="leaveModalOpen = false">
        <div class="modal-container">
            <div class="modal-header"><h2>New Leave Request</h2><button class="modal-close" @click="leaveModalOpen = false">✕</button></div>
            <div class="modal-body">
...
function dashboardApp() {
    return {
        ...
        prevMonth() { this.currentDateObj = new Date(this.year, this.month - 1); this.selectedDay = null; },
        nextMonth() { this.currentDateObj = new Date(this.year, this.month + 1); this.selectedDay = null; },
