            ...
            <div class="settings-grid">
                ...
                <div class="settings-card">
                    ...
                    <div class="setting-row">
                        ...
                        <span>
                            ...
                            <span
                                ...
                                :class="sheetsStatus"
                                x-text="sheetsStatus === 'connected' ? 'Connected' : (sheetsStatus === 'checking' ? 'Checking...' : 'Disconnected')"
                            ></span>
                        </span>
                    </div>
                    ...
                    <div class="settings-actions">
                        <button
                            ...
                            :disabled="sheetsStatus === 'checking'"
                        >
                            <span x-show="sheetsStatus !== 'checking'">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <button class="btn-outline" @click="clearCache()">
                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    d="M3 6h18M8 6V4h8v2m-9 0l1 15h8l1-15M10 11v6m4-6v6"/>
                            </svg>
                            Clear Cache
                        </button>
                    </div>
                    ...
                    <button
                        class="btn-primary"
                        @click="refreshData()"
                        style="margin-top:8px; display:inline-flex; align-items:center; justify-content:center; gap:7px;"
                    >
                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M4 4v5h5M20 20v-5h-5M5.5 9A7 7 0 0117 5.5L20 9M18.5 15A7 7 0 017 18.5L4 15"/>
                ...
                <div class="settings-card">
                    ...
                    <div class="setting-row">
                        ...
                        <input
                            type="number"
                            x-model="settings.sessionTimeout"
                            @change="saveSetting('sessionTimeout')"
                            min="5"
                            max="1440"
                        >
                    </div>
                    ...
                    <div class="setting-row">
                        ...
                        <label class="switch">
                            <input
                                type="checkbox"
                                x-model="settings.forceHTTPS"
                                @change="saveSetting('forceHTTPS')"
                            >
                ...
                <div class="settings-card">
                    ...
                    <div class="setting-row">
                        ...
                        <input
                            type="text"
                            x-model="settings.defaultLocation"
                            @change="saveSetting('defaultLocation')"
                            style="width:150px;"
                        >
                    </div>
                </div>
...
function settingsApp() {
    return {
        ...
        async testConnection() {
            ...
            try {
                const response = await fetch(
                    '/api/test-sheets-connection?_=' + Date.now()
                );

                const data = await response.json();

                this.sheetsStatus = data.connected
                    ? 'connected'
                    : 'disconnected';
        ...
        async clearCache() {
            if (!confirm(
                'Clear cache and refresh data from Google Sheets?'
            )) {
                return;
            }
            ...
            try {
                ...
                if (data.success) {
                    ...
                    window.appUtils.showToast(
                        'Failed to clear cache: ' + data.error,
                        'error'
                    );
                }
        ...
        async refreshData() {
            try {
                ...
                if (data.success) {
                    ...
                    window.appUtils.showToast(
                        'Failed to refresh data: ' + data.error,
                        'error'
                    );
                }
                ...
                window.appUtils.showToast(
                    'Error refreshing data: ' + err.message,
                    'error'
                );
            }
        },
        ...
        exportSettings() {
            ...
            a.href = url;
            a.download =
                `spysee_settings_${new Date().toISOString().split('T')[0]}.json`;
